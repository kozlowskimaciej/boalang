...
std::string code = R"V0G0N(
  ...
  EXPECT_TRUE(str_contains(e.what(), "Invalid contained value type cast"));
  throw;
  }
  },
  RuntimeError);
...
EXPECT_TRUE(str_contains(e.what(), "Tried to initialize 'v' with value of different type"));
throw;
}
},
RuntimeError);
...
std::string code = R"V0G0N(
  ...
  EXPECT_THROW(
  {
    ...
    EXPECT_TRUE(str_contains(e.what(), "Inspect did not match any types and default not present"));
    throw;
    }
  },
  RuntimeError);
...
std::string code = R"V0G0N(
  ...
  EXPECT_TRUE(str_contains(e.what(), "Cannot inspect non-variant objects"));
  throw;
  }
  },
  RuntimeError);
