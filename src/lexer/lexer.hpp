...
class Lexer {
 private:
  Source& source_;
  [[nodiscard]] Token build_token(const TokenType& type, const token_value_t& value = std::nullopt) const;
  ...
  public:
    LexerError(const Token& token, const std::wstring& message) :
        message_(L"Line " + std::to_wstring(token.position.line) + L" column " +
                     std::to_wstring(token.position.column) + L" at '" + token.stringify() + L"': " + message) {
